<!-- Edit -->
<div class="modal fade" id="edit{{ $date->slug }}">
    <div class=" modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title"><b>{{__('global.edit')}} {{__('global.dat')}}</b></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>

            </div>
            <div class="modal-body text-left">
                <form class="form-horizontal" method="POST" action="{{ route('date.update', $date->slug) }}">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        {{-- <div class="bootstrap-timepicker">
                            <input type="date" placeholder="dd-mm-yyyy" class="form-control timepicker" id="edit_date" name="date"
                                value="{{ $schedule->date }}">
                        </div> --}}
                        <div class="bootstrap-timepicker">
                            <input type="date" placeholder="dd-mm-yyyy" class="form-control timepicker" id="edit_date" name="date" />
...
<!-- Delete -->
<div class="modal fade" id="delete{{ $date->slug }}">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header " style="align-items: center">
               
                <h4 class="modal-title "><span class="employee_id">{{__('global.delete')}} {{__('global.dat')}}</span></h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              </div>
            <div class="modal-body">
                <form class="form-horizontal" method="POST" action="{{ route('date.destroy', $date->slug) }}">
                    @csrf
                    {{ method_field('DELETE') }}
                    <div class="text-center">
                        <h6>{{__('global.areYouSure')}} :</h6>
                        <h2 class="bold del_employee_name">{{ $date->slug}}</h2>
                    </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-flat pull-left" data-dismiss="modal"><i
